...
int Map::battle(char symbol1, char symbol2) 
{
    ...
    // Wyszukujemy obie istoty po symbolu
    ...
    while(entity1->isAlive() && entity2->isAlive()) 
    {
        ...
        if(killed)
        {
            ...
            return -1; // entity2 wygrała
        }

        round++;
        std::cout << "Naciśnij ENTER, aby kontynuować kolejną rundę...\n";
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');  // czyścimy bufor wejścia
        std::cin.get();  // czekamy na naciśnięcie ENTER
    }
    return 0;
}
