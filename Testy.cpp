...
// Zmienna globalna testMap – upewnij się, że nie koliduje z innymi częściami programu
Map* testMap = nullptr;
...
void Testy::createMap() 
{
    ...
    delete testMap;
    // Dodajemy 2 do wymiarów, aby uwzględnić ściany
...
{
    // Tworzymy małą mapkę dla testu walki
    Map mapka(4, 4, START_ROOM);
    
    // Tworzymy bohatera i potwora na ustalonych pozycjach
    ...
    // Rozpoczynamy walkę – zakładamy, że metoda battle przyjmuje symbole walczących istot
...
void Testy::testMonsters() 
{
    ...
    // Pobieramy listę potworów (zakładamy, że getMonsters() zwraca kontener z potworami)
    for (const auto& monster : potwory.getMonsters()) {
        if (monster.getSymbol() == monsterSymbol) {
            foundMonster = true;
            // Przeprowadzamy 5 pojedynków
            for (int i = 0; i < 5; ++i) {
                Istota hero("Bohater", 'H', 2, 2, 20, 6, 3);
                mapka.placeEntity(hero, 2, 2);
                // Zakładamy, że istnieje metoda placeMonster(), która umieszcza potwora na mapie
